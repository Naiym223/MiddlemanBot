...
const AcceptFriendReq = await rbx.acceptFriendRequest(Number(UserID)).catch(err => {BotMessage.edit({content: "Friend Request does not exist"})})
...
const AcceptedFriend = new Discord.MessageEmbed()
.setTitle(`Accepted ${args[0]} as a friend`)
.addField("Username: ", `${args[0]}`, true)
.addField("UserID: ", `${UserID}`, true)
.setColor("#f1c40f")
.setThumbnail(Thumbnail[0].imageUrl)
BotMessage.edit({embeds: [AcceptedFriend], content: null, }) 
}
    })
    })
})
